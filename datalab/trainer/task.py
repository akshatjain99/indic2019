...
FLAGS.tf_random_seed = 1985111
FLAGS.model_name = 'yolo-02'
...
model_dir = 'gs://indic2019/model_dir/run_akshat_3'
...
#     image= tf.reshape(image, (-1, img_w, img_h, img_channels), name='serve_reshape')
...
#    receiver_tensor = {'image_bytes': tf.placeholder(dtype=tf.string, shape=())}
#    features = {'images': tf.map_fn(preprocess, receiver_tensor['image_bytes'])}
...
train_spec = tf.estimator.TrainSpec(input_fn= lambda:input_fn(files_pattern, batch_size, mode=tf.estimator.ModeKeys.TRAIN),max_steps=100)

eval_spec = tf.estimator.EvalSpec(lambda: input_fn(files_pattern,  batch_size , mode=tf.estimator.ModeKeys.EVAL),steps=20, exporters=exporter)
...
export_dir = 'saved_model/1560258264/saved_model.pb'
estimator.export_savedmodel(export_dir, serving_input_fn, strip_default_attrs=True)
