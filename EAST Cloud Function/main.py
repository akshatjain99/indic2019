...
def predict(file_name_image, image_name):
    ...
    img_file = file_name_image
    file_name_image, headers_image = urllib.request.urlretrieve(file_name_image)
    ...
    if boxes is None:
        img_predict = img[:, :, ::-1]
        cv2.imwrite(output_image, img_predict)
        return output_image
